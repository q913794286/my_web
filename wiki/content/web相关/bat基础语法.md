...
e Si%o wh!E){-T p0pause 运行此句会暂停批处理的执行并在屏幕上显示Press any key to
...
那么在执行f.bat时，%1就表示a:，这样format %1就相当于format
...
oh2p\#n0　　在实际应用中我们会把这条命令和重定向符号（也称为管道符号，一般用\>
...
{TSC1U3s9xW0　　\@format X: /q/u/autoset (format
...
　　在这个例子中，驱动器 A
中磁盘上的所有文件均复制到d:”back中。显示的注释提示您将另一张磁盘放入驱动器 A
时，pause 命令会使程序挂起，以便您更换磁盘，然后按任意键继续处理。
...
jG&\@!e”he,NPr0　　指定要调用的批处理程序的位置和名称。filename 参数必须具有
.bat 或 .cmd 扩展名。
